        ...
        {user && (
          <>
                ...
                {editing && (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="sr-only"
                    />
                    <span className="sr-only">{t('profile.editProfile')}</span>
                  </label>
                )}
              </div>
              <h2 className="text-headline-xl font-headline-xl text-on-surface">
                {editing ? form.display_name || user.username : user.display_name || user.username}
              </h2>
              <p className="text-body-md text-on-surface-variant">@{user.username}</p>
            </section>

            {editing && (
              <form onSubmit={save} className="mb-8 space-y-md rounded-[24px] bg-surface-container-lowest p-lg cloud-shadow">
                <div>
                  <p className="text-label-lg text-on-surface-variant">{t('addEntry.photo')}</p>
                ...
                <div className="flex gap-sm pt-xs">
                  ...
                  >
                    {isUploadingAvatar ? t('common.uploading') : update.isPending ? t('common.saving') : t('common.save')}
                  </button>
                </div>
                {update.error && (
            ...
            <section className="mb-8 rounded-[24px] bg-surface-container-lowest p-lg cloud-shadow">
              <div className="mb-md flex items-center justify-between">
                <h2 className="text-label-lg text-on-surface-variant">{t('friends.myFriends')}</h2>
                ...
                {(profile.friends || []).slice(0, 5).map((friend) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-label-sm text-on-surface-variant">@{friend.username}</p>
                    </div>
                  </Link>
                ))}
              </div>
...
function Avatar({ user, large = false }) {
  ...
  return (
    <span className={`flex ${classes} shrink-0 items-center justify-center rounded-full bg-secondary-container font-semibold text-secondary`}>
      {initials}
    </span>
  )
}
