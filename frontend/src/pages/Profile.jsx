  ...
  return <main className="mx-auto min-h-screen w-full max-w-md bg-background pb-32 text-on-background">
    ...
    <div className="px-container-margin pt-sm">
      ...
      {user && <>
        ...
        <section className="mb-8"><div className="mb-md flex items-center justify-between"><h2 className="text-headline-lg font-headline-lg text-on-surface">Recent entries</h2><Link to="/calendar" className="text-label-lg text-primary">See all</Link></div><div className="grid grid-cols-2 gap-md">{(profile.recent_entries || []).map((entry) => <Link key={entry.id} to={`/entries/new?date=${entry.date}`} className="flex min-h-[140px] flex-col justify-between rounded-[24px] bg-surface-container-lowest p-lg cloud-shadow"><div className="flex items-start justify-between"><span className="text-label-sm text-on-surface-variant">{formatDate(entry.date)}</span><span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-container text-lg">{moods[entry.mood] || '😐'}</span></div><p className="line-clamp-2 text-body-sm text-on-surface">{entry.text || entry.tags?.[0] || 'No note for this day.'}</p></Link>)}</div>{profile.recent_entries?.length === 0 && <p className="rounded-[24px] bg-surface-container-low p-lg text-center text-body-sm text-on-surface-variant">Your entries will appear here.</p>}</section>
