  ...
  return <main className="mx-auto min-h-screen w-full max-w-md bg-background pb-32 text-on-background">
    ...
    <div className="px-container-margin pt-sm">
      ...
      {user && <>
        ...
        {editing && <form onSubmit={save} className="mb-8 space-y-md rounded-[24px] bg-white p-lg cloud-shadow"><div><p className="text-label-lg text-on-surface-variant">Profile photo</p><p className="mt-xs text-body-sm text-on-surface-variant">Tap the pencil on your photo to choose an image.</p>{avatarStatus && <p role="status" className={`mt-xs text-body-sm ${avatarStatus === 'Photo ready to save.' ? 'text-primary' : avatarStatus === 'Uploading photo...' ? 'text-on-surface-variant' : 'text-error'}`}>{avatarStatus}</p>}{form.avatar_url && <button type="button" onClick={() => { setForm((current) => ({ ...current, avatar_url: '' })); setAvatarStatus('Photo will be removed when you save.') }} disabled={isUploadingAvatar} className="mt-sm text-label-lg text-primary disabled:opacity-50">Remove photo</button>}</div><label className="block text-label-sm text-on-surface-variant">Display name<input value={form.display_name} maxLength={60} onChange={(e) => setForm({ ...form, display_name: e.target.value })} className="mt-xs w-full rounded-xl bg-surface-container-low px-md py-sm text-body-md outline-none focus:ring-2 focus:ring-primary/20" required /></label><div className="flex gap-sm pt-xs"><button type="button" onClick={cancelEdit} disabled={isUploadingAvatar} className="flex-1 rounded-full bg-surface-container-highest py-sm text-label-lg text-on-surface-variant disabled:opacity-50">Cancel</button><button type="submit" disabled={update.isPending || isUploadingAvatar} className="flex-1 rounded-full bg-primary py-sm text-label-lg text-on-primary disabled:opacity-50">{isUploadingAvatar ? 'Uploading...' : update.isPending ? 'Saving...' : 'Save'}</button></div>{update.error && <p role="alert" className="text-body-sm text-error">{update.error.message}</p>}</form>}
        <section className="mb-8"><div className="mb-md flex items-center justify-between"><h2 className="text-headline-lg font-headline-lg text-on-surface">Recent entries</h2><Link to="/calendar" className="text-label-lg text-primary">See all</Link></div><div className="grid grid-cols-2 gap-md">{(profile.recent_entries || []).map((entry) => <Link key={entry.id} to={`/entries/new?date=${entry.date}`} className="flex min-h-[140px] flex-col justify-between rounded-[24px] bg-white p-lg cloud-shadow"><div className="flex items-start justify-between"><span className="text-label-sm text-on-surface-variant">{formatDate(entry.date)}</span><span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary-container text-lg">{moods[entry.mood] || '😐'}</span></div><p className="line-clamp-2 text-body-sm text-on-surface">{entry.text || entry.tags?.[0] || 'No note for this day.'}</p></Link>)}</div>{profile.recent_entries?.length === 0 && <p className="rounded-[24px] bg-surface-container-low p-lg text-center text-body-sm text-on-surface-variant">Your entries will appear here.</p>}</section>
        <section className="mb-8 rounded-[24px] bg-white p-lg cloud-shadow"><div className="mb-md flex items-center justify-between"><h2 className="text-label-lg text-on-surface-variant">Friends</h2><Link to="/friends" className="text-label-lg text-primary">Manage</Link></div><div className="space-y-sm">{(profile.friends || []).slice(0, 5).map((friend) => <div key={friend.id} className="flex items-center gap-sm"><Avatar user={friend} /><div><p className="text-body-md font-semibold text-on-surface">{friend.display_name || friend.username}</p><p className="text-label-sm text-on-surface-variant">@{friend.username}</p></div></div>)}</div>{profile.friends?.length === 0 && <p className="text-body-sm text-on-surface-variant">Add friends to share your days.</p>}</section>
        <section className="rounded-[24px] bg-white p-lg cloud-shadow"><h2 className="mb-md text-label-lg text-on-surface-variant">App settings</h2><div className="flex items-center justify-between border-t border-surface-container-low py-sm"><span className="flex items-center gap-sm text-body-md text-on-surface"><span className="material-symbols-outlined text-on-surface-variant">palette</span>Theme toggle</span><span className="text-label-sm text-on-surface-variant">Coming soon</span></div></section>
      </>}
    </div><BottomNav />
  </main>
}
