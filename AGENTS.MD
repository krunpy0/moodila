...
- **Основные возможности:**
  ...
  - 📳 **Тактильный отклик (Haptic Feedback):** Нативная тактильная отдача для мобильных устройств на базе стандартного Web Vibration API с graceful fallback для неподдерживающих браузеров (паттерны для навигации, настроений, сохранения, удаления, реакций и записи аудио с тумблером в профиле).
...
19. **`0019_hide_custom_entries_for_new_friends.sql`** — Изоляция записей с кастомной приватностью от новых друзей:
    - Триггер `trg_hide_custom_entries_for_new_friend` на таблице `friendships`: при переходе дружбы в статус `accepted` автоматически скрывает (`is_hidden = true` в `entry_friend_visibility`) все существующие записи обоих пользователей, у которых настроена кастомная видимость.
    - Одноразовый бэкфилл для существующих принятых дружб и записей с кастомной видимостью.
